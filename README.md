...
Pentru a porni aplicația, urmează pașii în ordinea de mai jos:

### Pas 1: Pornește MySQL Server
Asigură-te că MySQL Server este pornit și rulează pe portul 3306.

### Pas 2: Pornește Backend-ul (Spring Boot)
Deschide un terminal în folderul `OnlineShop` și rulează:
```bash
cd OnlineShop
mvn spring-boot:run
```

Așteaptă până când vezi mesajul `Started OnlineShopApplication` în consolă. Backend-ul va rula pe **http://localhost:8080**

### Pas 3: Pornește Frontend-ul (React)
Deschide un alt terminal în folderul `frontend` și rulează:
```bash
cd frontend
npm start
```

Aplicația React se va deschide automat în browser pe **http://localhost:3000**

### Verificare
- Backend API: **http://localhost:8080/api/products** (GET request pentru a vedea produsele)
- Frontend UI: **http://localhost:3000** (interfața utilizatorului)

**Notă**: Asigură-te că MySQL este pornit înainte de a porni backend-ul, altfel aplicația va eșua la conectarea la baza de date.
